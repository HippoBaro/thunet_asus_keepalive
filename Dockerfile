...
&& \
        ...
        ./Configure linux-mips32 no-asm shared no-rc4 no-ssl2 no-ssl3 && \
        CC=/musl-cross-make/output/bin/mipsel-linux-muslsf-gcc CXX=/musl-cross-make/output/bin/mipsel-linux-muslsf-g++ make install -j$(nproc) \
&& \
        cd / && rm -rf /src &&\
